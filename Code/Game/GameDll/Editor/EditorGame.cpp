...
void CEditorGame::SetPlayerPosAng(Vec3 pos,Vec3 viewDir)
{
	...
	if (pClActor)
	{
		...
		IEntity *pPlayerEntity = pClActor->GetEntity();
		if (pPlayerEntity)
		{
			pe_player_dimensions dim;
			dim.heightEye = 0;
			if (IPhysicalEntity *pPlayerPhysics = pPlayerEntity->GetPhysics())
			{
				pPlayerPhysics->GetParams( &dim );
				pos.z = pos.z - dim.heightEye;
			}
		}

		pPlayerEntity->SetPosRotScale( pos,Quat::CreateRotationVDir(viewDir),Vec3(1,1,1),ENTITY_XFORM_EDITOR );
	}
}
...
void CEditorGame::OnBeforeLevelLoad()
{
	EnablePlayer(false);
	ConfigureNetContext(true);
	auto createdRules = m_pGame->GetIGameFramework()->GetIGameRulesSystem()->CreateGameRules("SinglePlayer");
