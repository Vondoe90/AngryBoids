...
function VehicleBase.Server:OnHit(hit)
  ...
  local direction = hit.dir;
	...
	if(hit and hit.shooterId and hit.type ~= "fire") then
		--todo g_gameRules.game:SendHitIndicator(hit.shooterId);
	end
	...
	-- SP only
	if(g_localActorId and self:GetSeat(g_localActorId) and HUD) then
		HUD.DamageIndicator(hit.weaponId, hit.shooterId, direction, true);
	end
  	
	self.vehicle:OnHit(targetId, hit.shooterId, hit.damage, hit.pos, hit.radius, hit.typeId, explosion);
