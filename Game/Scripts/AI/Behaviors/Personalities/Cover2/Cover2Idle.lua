...
local Behavior = CreateAIBehavior("Cover2Idle","HBaseIdle",	
{
	...
	OnBulletRain = function(self, entity, sender, data)
		...
		if(AI.Hostile(entity.id, data.id))then
			...
			if(AI.GetTargetType(entity.id)==AITARGET_NONE) then
				...
				local closestCover = AI.GetNearestHidespot(entity.id, 3, 15, target:GetPos());
				if(closestCover~=nil) then
					AI.SetBeaconPosition(entity.id, closestCover);
				else
					AI.SetBeaconPosition(entity.id, target:GetPos());
				end
			else
				entity:TriggerEvent(AIEVENT_DROPBEACON);
			end
