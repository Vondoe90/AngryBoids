...
DestroyableObject =
{
	...
	Properties =
	{
		...
		object_Model 					= "objects/default/primitive_sphere.cgf",							-- Pre-destroyed model/submodel.
		ModelSubObject				= "",
	...
	if (pass) then
		...
		if(explosion.DelayEffect.bHasDelayEffect==1)then
			if(not self.FXSlot or self.FXSlot==(-1))then
				...
				AI.FreeSignal(10, "OnExplosionDanger", self:GetPos(), self.Properties.Explosion.Radius, self.id, g_SignalData)
...
MakeInterestingToAI(DestroyableObject, 1);
