...
function AdvancedDoor:OnReset()
	...
	if(not EmptyString(props.fileModel))then 	
 		self:LoadObject(3,props.fileModel); -- First load whole object in slot 3.
 	end;
 	self:DrawSlot(3,0); -- Make it invisible
	if(not EmptyString(props.fileModel)and not EmptyString(props.fileModel))then 	
		self:LoadSubObject(0,props.fileModel, props.ModelSubObject);
	end;
	if(not EmptyString(props.fileModel)and not EmptyString(props.DestroyedSubObject))then 	
...
function AdvancedDoor:Explode()
	...
	if(NumberToBool(self.Properties.Destruction.bExplode))then
		...
		g_gameRules:CreateExplosion(self.shooterId,self.id,expl.Damage,explo_pos,expl.Direction,expl.Radius,nil,expl.Pressure,expl.HoleSize,expl.Effect,expl.EffectScale);	
